...
血糖知识图谱/
├── index.html                 # 工具箱首页
├── content/                   # Quartz 笔记源（Markdown）
├── site/                      # 交互式 HTML 工具
├── data/                      # 数据层
│   ├── foods/
│   │   ├── schema.json        # 食物数据模型定义
│   │   └── gi-database.json   # 145 种食物 GI/营养数据
│   └── knowledge/
│       ├── topics.json        # 12 个血糖知识主题
│       └── script-topics.json # 口播选题库
├── src/                       # Python 源码
│   ├── templates/prompts.json
│   └── generate_prompts.py
├── reports/methodology/       # 方法论报告
└── out/prompts/               # 生成产物（本地，不提交）
```
