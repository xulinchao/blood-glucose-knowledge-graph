...
```text
任务1（每日 09:00）：scripts/run-daily-harvest.bat  → 含精选日报
  输出：data/exports/daily-harvest-YYYY-MM-DD.json
...
任务2（手动/Agent，依赖任务1）：读精选 JSON → 写监控日报 → 更新 content/research-daily 若需发笔记
  前置条件：data/exports/daily-brief-YYYY-MM-DD.json 必须存在
  如果该文件不存在，Agent 不得手写日报覆盖，应提示用户先运行任务1
```

### TraeWork 监控日报强制流程

Agent 执行监控日报任务时，**必须严格按以下顺序**，不可跳步：

1. **检查前置**：确认 `data/exports/daily-brief-YYYY-MM-DD.json` 存在，不存在则中止并提示用户先跑 `scripts/run-daily-harvest.bat`
2. **先读** 当日 `data/exports/daily-brief-YYYY-MM-DD.json`（精选），再按需读 `daily-harvest-*.json`
3. **必须** 对引用主张做对抗性审核（见上节）；二手信息标「待核实」
4. **不得** 把 `hook_only` / E 级条目写成已核实事实
5. 监控日报：**HTML** 仅 `research-daily/*-日报.html`；**MD** 仅 `content/research-daily/*-日报.md`（Quartz）。不要在 `research-daily/` 留监控日报的 `.md`
6. 产出 HTML 首行保留 `<!-- Generated by Trae Work -->` 约定

> **2026-07-01 教训**：Agent 曾跳过 `daily_topic_harvest.py` 直接手写日报 HTML，导致 `data/exports/` 缺失 JSON 底座，精选日报无 Schema 2.0 字段（`writing_safety`/`gate`/`propagation`/`cognitive_conflict`），采集日报未走 `claim_gate.py` 评分。**此行为已明确禁止。**

TraeWork 系统提示词建议附加：

```text
工作目录：血糖知识图谱仓库根目录。遵守 AGENTS.md「选题与内容真实性」。
监控日报前置条件：data/exports/daily-brief-YYYY-MM-DD.json 必须存在，不存在则拒绝执行。
查今日热点先读 data/exports/latest-brief.json（见 blood-glucose-daily-brief Skill）。
不得跳过 daily_topic_harvest.py 手写日报——采集日报和精选日报必须由脚本生成。
```
