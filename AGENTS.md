...
- `src/generate_script.py` — CLI 口播文案生成（选题 + GI/知识库增强 + 归档）
...
本仓库面向**血糖/控糖科普**，维护者**非执业医生**。Agent 处理选题、日报、写稿、采集时**必须**遵守以下规则（与 `src/daily_topic_harvest.py` 的对抗性审核一致）。
...
`discovered-topics.json` 仅收录同时满足：证据非 E、`use_as` 可写稿、选题分≥62、真实性≥50。  
...
TraeWork 与本仓库 Cursor/Agent **共用同一套数据与规则**（`AGENTS.md` + `.cursor/skills/blood-glucose-topic-authenticity/SKILL.md`）。

### 两类日报（勿混淆）

| 类型 | 生成方式 | 输出 | 用途 |
|------|----------|------|------|
| **采集日报** | 定时跑 `scripts/run-daily-harvest.bat` | `research-daily/YYYY-MM-DD-采集日报.html` | 多平台原贴 + 证据层级 + 对抗性标记 |
| **监控日报** | TraeWork 任务（深度解读） | `research-daily/YYYY-MM-DD-日报.html` | 政策/指南核实、选题脚本建议 |

TraeWork 写监控日报时：

1. **先读** 当日 `data/exports/daily-harvest-*.json` 或 `*-采集日报.md`，不要重复采集
2. **必须** 对引用主张做对抗性审核（见上节）；二手信息标「待核实」
3. **不得** 把 `hook_only` / E 级条目写成已核实事实
4. 产出 HTML 首行保留 `<!-- Generated by Trae Work -->` 约定

### TraeWork 推荐定时任务

```text
任务1（每日）：scripts/run-daily-harvest.bat
任务2（可选，TraeWork Agent）：阅读采集日报 → 写监控日报 → 更新 content/research-daily 若需发笔记
```
